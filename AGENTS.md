...
- **Manual recording refresh**: `./internal/refresh_recordings.sh`
  ...
  - Requires every hosted-provider credential plus authenticated `claude`, `codex`, `opencode`, and `pi` CLIs. It is intentionally separate from the weekly model-list job because new models can require scoreboard updates.
