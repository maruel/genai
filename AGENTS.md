...
- **Weekly model regeneration**: `./scripts/regen_weekly.sh`
  ...
  - Required env vars: `ANTHROPIC_API_KEY`, `BASETEN_API_KEY`, `CEREBRAS_API_KEY`, `CLOUDFLARE_ACCOUNT_ID`, `CLOUDFLARE_API_KEY`, `COHERE_API_KEY`, `DEEPSEEK_API_KEY`, `GEMINI_API_KEY`, `GROQ_API_KEY`, `HUGGINGFACE_API_KEY`, `MISTRAL_API_KEY`, `OPENAI_API_KEY`, `OPENROUTER_API_KEY`, `TOGETHER_API_KEY`, `TYPESAFE_API_KEY`, `MIMO_API_KEY`, and one of `DASHSCOPE_API_KEY_INTL`, `DASHSCOPE_API_KEY_US`, `DASHSCOPE_API_KEY_CN`, or `DASHSCOPE_API_KEY`.
...
- **Manual recording refresh**: `./scripts/refresh_recordings.sh [--from PROVIDER]`
  - Re-records every provider `Warmup.yaml` and `.ndjson` fixture, updates scoreboards for model-list changes, and regenerates generated documentation.
  - Uses a 60-minute per-provider test timeout because live reasoning and subprocess recordings can exceed Go's 10-minute default.
  - Use `--from PROVIDER` to resume after a failed provider without deleting or requiring credentials for earlier providers.
  - Requires credentials and authenticated CLIs for each provider it runs. It is intentionally separate from the weekly model-list job because new models can require scoreboard updates.
...
- `.github/workflows/test.yml`: Run tests and linters on main and release changes, pull requests, and automation requests.
- `.github/workflows/weekly-model-regen.yml`: Weekly model regeneration workflow.
- `.golangci.yml`: golangci-lint configuration.
- `Makefile`: Build, test, lint, and format the repository.
- `README.md`: genai
...
- `scripts/refresh_recordings.sh`: Script to refresh provider model-list and subprocess recordings.
- `scripts/regen_weekly.sh`: Script to regenerate weekly model scores for all providers.
...
- `smoke/smoketest/smoketest_test.go`: Tests for the smoke test scoreboard updater.
