...
func ExampleNew_hTTP_record() {
	...
	wrapper := func(h http.RoundTripper) http.RoundTripper {
		...
		m := cassette.NewDefaultMatcher(cassette.WithIgnoreHeaders("X-Api-Key", "X-Request-Id"))
		var err error
	...
	c, err := anthropic.New(&genai.OptionsProvider{APIKey: apiKey, Model: base.NoModel}, wrapper)
	if err != nil {
		log.Fatal(err)
	}
