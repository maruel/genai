...
func TestMagistralThinking_GenSync(t *testing.T) {
	tests := []struct {
		name    string
		tagName string
		in      string
		...
		},
		{
			...
			in:   "  \n\t<think>\nThinking with whitespace before\n\\boxed{\nResponse}",
			want: []genai.Content{
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			tp := &mistral.MagistralThinking{ProviderGen: mp}
...
func TestMagistralThinking_GenSync_errors(t *testing.T) {
	tests := []struct {
		name    string
		tagName string
		in      []genai.Content
	...
	}{
		{
			...
			in:   []genai.Content{{Text: "Text before <think>\nThis is thinking\n\\boxed{\nThis is response}"}},
		...
		},
		{
			...
			in:   []genai.Content{{Text: "First part. "}, {Text: "<think>Thinking part\n\\boxed{"}, {Text: " Second part."}},
...
func TestMagistralThinking_GenStream(t *testing.T) {
	tests := []struct {
		name string
		in   []string
		opts genai.Options
		...
		},
		{
			...
			in:   []string{"<think>", "This is my ", "thinking process", "\n\\boxed{", "This is the response}"},
			want: []genai.Content{
				{Thinking: "This is my thinking process"},
				{Text: "This is the response"},
			},
		...
		},
		{
			...
			in:   []string{"<think>", "Thinking1", "Thinking2\n\\boxed{", "Response", "}"},
			want: []genai.Content{
		...
		},
		{
			...
			in:   []string{"<think>", "Thinking content", "\n\\boxed{", "Response", "}"},
			want: []genai.Content{
				{Thinking: "Thinking content"},
				{Text: "Response"},
			},
...
func TestMagistralThinking_GenStream_errors(t *testing.T) {
	tests := []struct {
		name      string
		in        []string
	...
	}{
		{
			...
			in:   []string{"Text before <think>", "This is thinking", "\n\\boxed{", "This is response", "}"},
			want: "unexpected prefix before thinking tag: \"Text before\"",
		},
		{
