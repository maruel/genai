...
func getClientRT(t testing.TB, model scoreboardtest.Model, fn func(http.RoundTripper) http.RoundTripper) genai.Provider {
	...
	c, err := anthropic.New(apiKey, model.Model, fn)
	if err != nil {
		t.Fatal(err)
	}
	if model.Thinking {
	...
	data := []internaltest.ProviderError{
		{
			...
			ErrListModel: "http 401: error authentication_error: invalid x-api-key. You can get a new API key at https://console.anthropic.com/settings/keys",
		},
		{
			Name:         "bad model",
			Model:        "bad model",
