...
func getClientRT(t testing.TB, model scoreboardtest.Model, fn func(http.RoundTripper) http.RoundTripper) genai.Provider {
	...
	c, err := openaichat.New(&genai.OptionsProvider{APIKey: apiKey, Model: model.Model}, fn)
	if err != nil {
		t.Fatal(err)
	}
	if model.Thinking {
	...
	data := []internaltest.ProviderError{
		{
			...
			APIKey:       "bad apiKey",
			Model:        "gpt-4.1-nano",
			ErrGenSync:   "http 401\ninvalid_api_key (): Incorrect API key provided: bad apiKey. You can find your API key at https://platform.openai.com/account/api-keys.",
